...
public class JWTRefreshEndpoint extends AssignmentEndpoint {
  ...
  // Map refresh tokens to the user they were issued for
  ...
  private Map<String, Object> createNewTokens(String user) {
    ...
    // Bind the refresh token to the user it was issued for
  ...
  public ResponseEntity newToken(
    ...
    if (user == null || refreshToken == null) {
      ...
      // Verify that the refresh token belongs to the user in the JWT
      String tokenOwner = validRefreshTokens.get(refreshToken);
      if (!user.equals(tokenOwner)) {
        // Refresh token does not belong to this user - potential attack
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
      }
